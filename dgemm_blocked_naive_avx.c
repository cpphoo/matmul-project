 ...
 *  C := C + A * B
 * where C is ?-by-?, A is ?-by-?, and B is ?-by-?
 * using naive AVX instructions.
 */
void naive_matrix_block_multiply(double* restrict A, double* restrict B, double* restrict C, int block_size, int MM, int NN, int KK) {
    int i, j, k;
    __m256d sum, left, right;
    for (i = 0; i < MM; i+=4) {
        for (j = 0; j < NN; ++j) {
            sum = _mm256_load_pd(&C[i+j*block_size]);
            for (k = 0; k < KK; ++k) {
                left = _mm256_load_pd(&A[i+k*block_size]);
                right = _mm256_broadcast_sd(&B[k+j*block_size]);
                sum = _mm256_fmadd_pd(left, right, sum);
            }
            _mm256_storeu_pd(&C[i+j*block_size], sum);
        }
    }
}
